...
* **Azure:** Create a Function App _(for this particular demo, one running NodeJS 22 -- and I tested my code with a Linux one, not a Windows one)_ and its storage account _(and grant the Function App appropriate permissions against its underlying storage account if necessary, using Azure RBAC Role Assignments)_.  Grant the Entra service principal mentioned above an Azure RBAC Role Assignment of `Website Contributor` scoped against the Function App, and another one of `Storage Blob Data Contributor` scoped against the storage account.
